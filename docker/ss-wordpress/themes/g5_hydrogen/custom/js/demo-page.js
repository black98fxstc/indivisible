	...
	function browserFindMe () {
		...
		if (output && !navigator.geolocation)
		{
	...
	function getLowerDox (leg_id) {
	    ...
	    xmlhttp.onreadystatechange = function() {
			if (this.readyState == 4 && this.status == 200) {
			    ...
			    if (dox.url)
			    	jQuery("#lower-website").append(jQuery("<a>", { href: dox.url, target: "_blank" }).text("Website"));
			    jQuery("#lower-actions").empty();
		    	jQuery("#lower-actions").append(jQuery("<a>", { href: "/legislator/" + dox.leg_id.toLowerCase() }).text("Actions"));
		    };
	    }
	...
	function getUpperDox (leg_id) {
	    ...
	    xmlhttp.onreadystatechange = function() {
			if (this.readyState == 4 && this.status == 200) {
			    ...
			    if (dox.url)
			    	jQuery("#upper-website").append(jQuery("<a>", { href: dox.url, target: "_blank" }).text ("Website"));
			    jQuery("#upper-actions").empty();
		    	jQuery("#upper-actions").append(jQuery("<a>", { href: "/legislator/" + dox.leg_id.toLowerCase() }).text("Actions"));
		    };
	    }
